...
os.environ['WANDB_DIR'] = '/nas/ckgfs/users/thawani/etok/'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--codebook_groups", type=str, default="32,32,32") # "256,256,256"
    ...
    print(args.codebook_groups)
    wrun = wandb.init(config=args, project="factorizer")
    wrun.name = f"{args.codebook_groups.split(',')[0]}_{args.seed}_{wrun.name}"
    if TRAIN_NUM:
        wrun.name = f"num_{wrun.name}"
    elif TRAIN_AR: 
        wrun.name = f"ar_{wrun.name}"
    #wandb.init(name=f"{args.language}_regularized_{args.hidden_size}_{args.seed}", config=args, project="factorizer")
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    if TRAIN_AR:
        train_dataset = Dataset(args, path='/nas/ckgfs/users/thawani/etok/ar_factorizer_data/train.tsv', vocab=None, random=True, split=True)
        valid_dataset = Dataset(args, path='/nas/ckgfs/users/thawani/etok/ar_factorizer_data/test.tsv', vocab=None, random=True, split=True)
        frequent_dataset = Dataset(args, path='/nas/ckgfs/users/thawani/etok/ar_factorizer_data/10k.txt', vocab=None, random=True, split=True, remove_long_words=True)
    elif TRAIN_NUM:
        train_dataset = Dataset(args, path='/nas/ckgfs/users/thawani/etok/num_factorizer_data/numtrain.txt', vocab=None, random=True, split=True) # 300k
        valid_dataset = Dataset(args, path='/nas/ckgfs/users/thawani/etok/num_factorizer_data/numtest.txt', vocab=None, random=True, split=True) # 10k
        frequent_dataset = Dataset(args, path='/nas/ckgfs/users/thawani/etok/num_factorizer_data/num10k.txt', vocab=None, random=True, split=True, remove_long_words=True)
    else:
        train_dataset = Dataset(args, path='/nas/ckgfs/users/thawani/etok/count_1w_train.txt', vocab=None, random=True, split=True) # 300k
        valid_dataset = Dataset(args, path='/nas/ckgfs/users/thawani/etok/count_1w_test.txt', vocab=None, random=True, split=True) # 33k
        frequent_dataset = Dataset(args, path='/nas/ckgfs/users/thawani/etok/count_1w_10k.txt', vocab=None, random=True, split=True, remove_long_words=True)
    
    def WeightedRandomSampler(weights, num_samples, replacement=True):
        return torch.utils.data.sampler.WeightedRandomSampler(weights, num_samples, replacement=True)
    ...
    print('epochs:', args.num_steps // args.steps_per_epoch)
    # train loop
    for epoch in range(args.num_steps // args.steps_per_epoch):
        ...
        with torch.no_grad():
            ...
            try:
                if not TRAIN_AR and not TRAIN_NUM:
