...
if __name__ == "__main__":
    ...
    train_dataset = Dataset(args, path='factorizer/train.txt', vocab=None, random=True, split=True) # 300k
    valid_dataset = Dataset(args, path='factorizer/val.txt', vocab=None, random=True, split=True) # 33k
    frequent_dataset = Dataset(args, path='factorizer/most_frequent.txt', vocab=None, random=True, split=True, remove_long_words=True)
    ...
    for epoch in range(args.num_steps // args.steps_per_epoch):
        ...
        for step, (words, freqs) in enumerate(train_iter):
            ...
            ema_model.update()
            print('perplexity : ', vq.get_codebook_perplexity())
            wandb.log(
                {
                    "epoch": epoch,
